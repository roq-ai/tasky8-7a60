...
function TaskCreatePage() {
  ...
  const handleSubmit = async (values: TaskInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await roqClient.task.create({ data: values as RoqTypes.task });
      resetForm();
      router.push('/tasks');
    } catch (error) {
      setError(error);
  ...
  const formik = useFormik<TaskInterface>({
    initialValues: {
      name: '',
      status: false,
      list_id: (router.query.list_id as string) ?? null,
    },
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <AsyncSelect<ListInterface>
            formik={formik}
            name={'list_id'}
            label={'Select List'}
            placeholder={'Select List'}
            fetcher={() => roqClient.list.findManyWithCount({})}
            labelField={'id'}
          />
