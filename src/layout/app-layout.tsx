...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiList, FiCheckSquare } from 'react-icons/fi';

import { useTeamFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'viewer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useTeamFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Teams',
      path: isTenantUser ? `/teams/view/${data?.id}` : '/teams',
      entity: 'team',
      service: AccessServiceEnum.PROJECT,
      icon: FiUsers,
    },
    { name: 'Lists', path: '/lists', entity: 'list', service: AccessServiceEnum.PROJECT, icon: FiList },
    { name: 'Tasks', path: '/tasks', entity: 'task', service: AccessServiceEnum.PROJECT, icon: FiCheckSquare },
