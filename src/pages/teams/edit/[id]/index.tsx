...
import { useRoqClient, useTeamFindFirst } from 'lib/roq';
...
function TeamEditPage() {
  ...
  const { data, error, isLoading, mutate } = useTeamFindFirst(queryParams, {}, { disabled: !id });
  ...
  const handleSubmit = async (values: TeamInterface, { resetForm }: FormikHelpers<any>) => {
    ...
    try {
      ...
      const updated = await roqClient.team.update({
        data: values as RoqTypes.team,
        where: {
          id,
        },
      });
