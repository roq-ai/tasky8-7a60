...
import { useRoqPlatformUserProfiles, useTaskCount, useListCount } from 'lib/roq';

import { ListListPage as ShowCaseList } from 'pages/lists';

function DashboardPage() {
  const { session } = useSession();

  const taskStat = useTaskCount();

  const listStat = useListCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Tasks',
      result: taskStat?.data,
      isLoading: taskStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Lists',
      result: listStat?.data,
      isLoading: listStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`lists`} passHref legacyBehavior>
                  <Button
