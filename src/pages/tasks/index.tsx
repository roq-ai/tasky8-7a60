...
import { useRoqClient, useTaskFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<TaskInterface, unknown>;
...
export function TaskListPage(props: TaskListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useTaskFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.task.delete({ where: { id } });
      await mutate();
    } catch (error) {
    ...
    hasAccess('list', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/lists/view/${record.list?.id}`}>
              {record.list?.id}
            </Link>
          ),
        }
